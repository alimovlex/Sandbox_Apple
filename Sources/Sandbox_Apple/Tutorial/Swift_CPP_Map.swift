...
open class SwiftMapWrapper {
    ...
    func begin() -> SwiftMapIterator {
        return SwiftMapIterator(cppObject: Sandbox_CPP.map_begin(cppObject), dataSize: dataSize)
    }

    // – Returns an iterator pointing to the theoretical last element which follows the last element
    func end() -> SwiftMapIterator {
        return SwiftMapIterator(cppObject: Sandbox_CPP.map_end(cppObject), dataSize: dataSize)
    }
