...
open class ByteBackpacker {
    ...
    open class func unpack<T: Any>(_ valueByteArray: [UInt8], byteOrder: ByteOrder = .nativeByteOrder) -> T {
    ...
    ///   - byteOrder: Byte order (wither little or big endian)
    /// - Returns: Value type of type `T`
    open class func unpack<T: Any>(_ valueByteArray: [UInt8], toType type: T.Type, byteOrder: ByteOrder = .nativeByteOrder) -> T {
    ...
    ///   - byteOrder: Byte order (wither little or big endian)
    /// - Returns: Byte array
    open class func pack<T: Any>( _ value: T, byteOrder: ByteOrder = .nativeByteOrder) -> [UInt8] {
        assert(!(T.self is AnyClass), ByteBackpacker.referenceTypeErrorString)
