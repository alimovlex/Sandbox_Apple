...
open class SwiftListIterator {
    ...
    func value() -> Data? {
        if let p:UnsafeMutableRawPointer = UnsafeMutableRawPointer(mutating: Sandbox_CPP.list_value(cppObject)) {
    ...
    func increment() -> Bool {
        return Sandbox_CPP.list_increment(cppObject)
    }

    // – Decrement iterator - return false if before begin and do not increment
    func decrement() -> Bool {
        return Sandbox_CPP.list_decrement(cppObject)
    }
}
...
open class SwiftListWrapper {
    ...
    func front() -> Data? {
        if let p:UnsafeMutableRawPointer = UnsafeMutableRawPointer(mutating: Sandbox_CPP.list_front(cppObject)) {
            let data = Data(bytes: p, count: dataSize)
            return data
        }
    ...
    func back() -> Data? {
        if let p:UnsafeMutableRawPointer = UnsafeMutableRawPointer(mutating: Sandbox_CPP.list_back(cppObject)) {
            let data = Data(bytes: p, count: dataSize)
            return data
        }
    ...
    func push_front(value: Data) {
        value.withUnsafeBytes {
            Sandbox_CPP.list_push_front(cppObject, $0)
        }
    }
    ...
    func push_back(value: Data) {
        value.withUnsafeBytes {
            Sandbox_CPP.list_push_back(cppObject, $0)
        }
    }

    // – Removes the first element of the list, and reduces size of the list by 1
    func pop_front() {
        Sandbox_CPP.list_pop_front(cppObject)
    }

    // – Removes the last element of the list, and reduces size of the list by 1
    func pop_back() {
        Sandbox_CPP.list_pop_back(cppObject)
    }

    // – Returns an iterator pointing to the first element of the list
    func begin() -> SwiftListIterator {
        return SwiftListIterator(cppObject: Sandbox_CPP.list_begin(cppObject), dataSize: dataSize)
    }

    // – Returns an iterator pointing to the theoretical last element which follows the last element
    func end() -> SwiftListIterator {
        return SwiftListIterator(cppObject: Sandbox_CPP.list_end(cppObject), dataSize: dataSize)
    }

    // – Returns whether the list is empty(1) or not(0)
    func empty() -> Bool {
        return Sandbox_CPP.list_empty(cppObject)
    }

    // – Returns the number of elements in the list
    func size() -> Int {
        return Sandbox_CPP.list_size(cppObject)
    }
}